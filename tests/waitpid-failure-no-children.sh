...
run_test -O "$DTEE_TEST_MONITOR_OUTPUT" ./test-lorem-ipsum-wait-exit 1
RET=$?

test_prepare
eval $(LD_PRELOAD="$TEST_LD_PRELOAD" ./test-waitpid ./dtee ./dtee -O "$DTEE_TEST_MONITOR_OUTPUT" ./test-lorem-ipsum-wait-exit 1)
RET2=$?
