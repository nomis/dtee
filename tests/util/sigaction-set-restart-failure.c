...
static int dtee_test___sigaction_siginfo_set_restart_failure(int signum, const dtee_sigaction_t *act, dtee_sigaction_t *oldact) {
...
TEST_FCN_REPL(int, __sigaction_siginfo, (int signum, const dtee_sigaction_t *act, dtee_sigaction_t *oldact)) {
	...
	if (!active) {
		...
		if (dtee_test_is_dtee()) {
			next___sigaction_siginfo = dtee_test___sigaction_siginfo_set_restart_failure;
		}
