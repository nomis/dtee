...
echo -n "I" | run_test_once \
	...
	-q ./test-report 'Hello World!' ETAOIN SHRDLU 42</dev/null
RET=$?
...
if [ $RET -eq 1 ] && [ $CMP_OUT_A -eq 0 ] && [ $CMP_OUT_O -eq 0 ] && [ $CMP_ERR_A -eq 0 ] && [ $CMP_ERR_O -eq 0 ] && [ $CMP_COM_A -eq 0 ] && [ $CMP_COM_O -eq 0 ]; then
	exit 0
fi
exit 1
